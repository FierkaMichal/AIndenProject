...
//    List<ConversationFlow> getAllMessagesByUsers(@Param("p_sender") long p_sender, @Param("p_receiver") long p_receiver);
    ...
    @Query("SELECT cf FROM ConversationFlow cf WHERE cf.conversationByIdConversation.idConversation = (SELECT c.idConversation FROM Conversation c WHERE ((c.userOne.idUser = :user1 AND c.userTwo.idUser = :user2) OR (c.userTwo.idUser = :user1 AND c.userOne.idUser= :user2)))order by cf.time desc")
